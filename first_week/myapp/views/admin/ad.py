...
def list_api(request: HttpRequest) -> APIResponse:
    ...
    返回:
        code=0 的 APIResponse，'data' 中包含分页的广告列表。
    """
    queryset = Ad.objects.all().order_by('-create_time')
    result = paginate_queryset(queryset, request)
    serializer = AdSerializer(result['page_obj'], many=True)
...
def create(request: HttpRequest) -> APIResponse:
    ...
    参数:
        request: 传入的 HTTP POST 请求，请求体中包含广告字段。

    返回:
        成功时返回 code=0 并附带创建的广告数据，
        验证失败时返回 code=1。
    """
    return create_instance(AdSerializer, request.data)
...
def update(request: HttpRequest) -> APIResponse:
    ...
    返回:
        成功时返回 code=0 并附带更新后的广告数据，
        未找到或验证失败时返回 code=1。
    """
    pk = request.GET.get('id', -1)
    try:
        ad = get_object_or_error(Ad, pk)
    except ValueError as e:
        return APIResponse(code=1, msg=str(e))

    return update_instance(AdSerializer, ad, request.data)
